...
import {
  ...
  subscribeToMonthlyCategorizedExpenses,
} from "@/utils/firebase"; // Import updated Firestore functions
...
const DashboardBudget: React.FC<{
  currency: string;
  ...
  if (budget) {
    ...
    const expense = categoryExpenses[budget.category]
      ? categoryExpenses[budget.category]
      : 0;
    const percentage = (expense / budget.amount) * 100;
    budgetComponent = (
      <Card key={budget.id} className="gap-0 bg-gray-100 dark:bg-gray-700">
            ...
            <div>
              <div className="flex items-center justify-between text-gray-800 text-base mb-1">
                ...
                <span className="ml-2 text-black dark:text-white font-semibold">
                  {getCurrencySymbol(currency) + expense?.toFixed(2)} /{" "}
  ...
  useEffect(() => {
    ...
    if (userId && status === "authenticated") {
      setLoading(true);
      subscribeToBudgetDashboard(userId, (budget) => {
        ...
        <CardContent className="space-y-4 text-gray-500 dark:text-gray-400">
          {!budget ? (
            <div className="text-center p-8 rounded-md">
              ...
              <p className="text-sm">
                You can pin one budget from the Budgets Page
              </p>
            </div>
          ) : (
            <div className="flex flex-col gap-4">{budgetComponent}</div>
          )}
        </CardContent>
      </Card>
    </>
