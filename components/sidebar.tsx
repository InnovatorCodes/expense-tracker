...
import { Home, DollarSign, HelpCircle, LogOut, Wallet, Menu, } from 'lucide-react';
...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <aside className={`z-50 sticky h-[100vh] top-0 w-sm bg-white dark:bg-gray-800 p-6 pt-6 flex-col sm:flex max-sm:fixed max-sm:top-0 max-md:min-h-[100vh] max-sm:w-70 ${isSidebarOpen?"":"max-sm:-translate-x-full"}`}>
      <div className="flex items-center gap-2 mb-10">
        <div className="bg-indigo-600 p-2 rounded-lg">
          <DollarSign className="text-white" size={28} />
        </div>
        <h1 className="text-4xl max-sm:text-3xl font-bold text-gray-800 dark:text-white sticky top-0">SpendSense</h1>
      </div>
      ...
      <form className="mt-auto max-sm:mt-2" action={signOut}>
         <Button className='text-xl bg-red-400 text-white w-full p-6 flex justify-start'>
          <LogOut style={{height: "30px", width: "30px"}}/>Logout
         </Button>
      </form>
    </aside>
    </>
